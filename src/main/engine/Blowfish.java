...
public class Blowfish implements Decryptor, Encryptor {
    ...
    public Blowfish(String mode, String key, byte[] iv, int operationMode) {
        ...
        if ((mode.equals("CBC") && operationMode == Cipher.DECRYPT_MODE) && (iv == null || iv.length != 8)) {
            throw new IllegalArgumentException("Blowfish CBC Decryption requires 8-bit iv vector.");
        }
        if (mode.equals("CBC") && operationMode == Cipher.ENCRYPT_MODE && iv != null) {
            throw new IllegalArgumentException("Blowfish CBC Encryption does not accept non-null value in IV.");
        }
