...
public class Blowfish implements Decryptor, Encryptor {

    private final String algorithmName = "Blowfish";
    ...
    public Blowfish(String mode, String key, byte[] iv, int operationMode) {
        ...
        if (((mode.equals("CBC") || mode.equals("CFB")) && operationMode == Cipher.DECRYPT_MODE) && (iv == null || iv.length != 8)) {
            throw new IllegalArgumentException("Blowfish CBC/CFB Decryption requires 8-bit iv vector.");
        }
        if ((mode.equals("CBC") || mode.equals("CFB")) && operationMode == Cipher.ENCRYPT_MODE && iv != null) {
            throw new IllegalArgumentException("Blowfish CBC/CFB Encryption does not accept non-null value in IV.");
        }
